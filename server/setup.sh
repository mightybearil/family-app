...
#   sudo ./setup.sh [DOMAIN_NAME]
#   Example: sudo ./setup.sh family.example.com
#
# ==============================================================================
...
# Add Node.js official setup repository if nodejs is outdated or missing
...
chown -R ubuntu:ubuntu "${APP_DIR}" || chown -R 1000:1000 "${APP_DIR}" || true
chmod 755 "${APP_DIR}"
...
server {
    ...
    # CORS Headers / הרשאות משאבים צולבים
...
# Setup Certbot SSL if domain is provided and not default example / הגדרת תעודת אבטחה SSL
if [ "$DOMAIN" != "family.example.com" ]; then
    log_info "Attempting SSL certificate issuance via Certbot for ${DOMAIN}..."
    certbot --nginx -d "$DOMAIN" --non-interactive --agree-tos --register-unsafely-without-email || log_warning "Certbot SSL issuance skipped or failed. Run certbot manually later."
fi
